...
- [ ] T002-S1 Add checkpoint encode/decode and file-backed save/load/list helpers for `ReplayState`, using the consensus/node snapshot codec proven by tests.
...
- [ ] T004-S1 Add focused unit tests proving checkpoint round-trip, nearest checkpoint selection, and replay-tail truncation behavior.
...
- [ ] T008-S2 On reachable non-origin rollback, rewind from checkpoint plus tail, truncate volatile tail state, and continue with `Progress`.
...
- [ ] T012-S3 Extend e2e replay coverage with a devnet checkpoint/rollback/replay-tail recovery proof.
- [ ] T013-S3 Verify recovered ledger state and latest recomputed tree/root match direct replay to the same point.
